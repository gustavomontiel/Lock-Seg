...
export class TitaniumService {
  titaniumURL: string = 'https://web.alarmhome.app/hsi_interface/user_api/v1';
  accessToken: string;
  appKey: string;
  appId: string;
  clientSecret: string;
  ...
  getWirelessZonesIdStartsFrom() {
    return this.urlsService.getParametro('idZonaInhalambrica')
  }
  ...
  async getToken() {    /*Obtiene el Token  */
    if (this.isTimeToLogin()) {
      const url = `${this.titaniumURL}/login`;
      this.accessToken = ''
  ...
  descifrarEstado(estado) { /* Convierte la respuesta de la API a algo comprensible */
    ...
    return { regProblemas, estadoParticiones, particionesListas, estadoSalidas, zonaAbierta, zonaAlarma, zonaInhibida }
  }
  ...
  async consultaPanel(numeroSistema: string, seq: string) {    /*Consulta de estado de Panel  */
    ...
    if (this.accessToken) {
      const url = `${this.titaniumURL}/panel_commands/state?numeroSistema=${numeroSistema}&seq=${seq}`;
      if (this.accessToken) {
        const headers = {
          "x-access-token": this.accessToken,
          "Content-Type": "application/json",
  ...
  async armarPanel(numeroSistema: string, seq: string, userNumber: string, numeroParticion: string) {    /*Arma la Partición  */
    const token = await this.getToken();
    if (this.accessToken) {
      const url = `${this.titaniumURL}/panel_commands/arm/away?numeroSistema=${numeroSistema}&seq=${seq}&numeroParticion=${numeroParticion}&userNumber=${userNumber}`;
      const headers = {
        "x-access-token": this.accessToken,
        "Content-Type": "application/json",
      };
  ...
  async desarmarPanel(numeroSistema: string, seq: string, userNumber: string, numeroParticion: string) {    /* Desarma la Partición  */
    const token = await this.getToken();
    if (this.accessToken) {
      const url = `${this.titaniumURL}/panel_commands/disarm?numeroSistema=${numeroSistema}&seq=${seq}&numeroParticion=${numeroParticion}&userNumber=${userNumber}`;
      const headers = {
        "x-access-token": this.accessToken,
        "Content-Type": "application/json",
      };
        ...
        this.http
          ...
          .then((data: any) => {
            ...
            if (data.result === 'OK') {
              ...
              this.toastService.presentToast('No se puede incluir la zona mientras la partición esta armada', 'danger');
            }
            return data;
          })
      );
        ...
        this.http
          ...
          .then((data: any) => {
            ...
            if (data.result === 'OK') {
              ...
              this.toastService.presentToast('No se puede excluir la zona mientras la partición esta armada', 'danger');
            }
            return data;
          })
      );
